...
静的サイトなので、ファイルを配信できればどのホストでも構いません。ドメインを向ける前に 4 点だけ確認してください。

* **`pnpm build` でビルド**（中身は `tsc -b && vite build`、出力は `dist/`）し、`dist/` を公開します。
  ビルドには Node 22.13+ が必要です。ホストのビルド環境がそれより古い場合は、ローカルでビルドして
  `dist/` の中身をアップロードしてください。
* **ドメインのルートに置く**（サブディレクトリ不可）。service worker、manifest、WebAssembly モジュールは
  `/`、`/wasm`、`/artifacts` から取得します。
* **SPA フォールバックを有効にする**: 未知のパスはすべて `index.html` を返す必要があります。そうでないと
  `/tools/patch/plan` のような深いリンクへの初回アクセスが 404 になります（一度開いた後は service worker が
  キャッシュしたシェルを返します）。Netlify / Cloudflare Pages なら `public/_redirects` に
  `/* /index.html 200`、Vercel なら `vercel.json` の rewrite、nginx なら `try_files $uri /index.html` です。
* **HTTPS を使う**: WebCrypto など一部のブラウザ API はセキュアコンテキストにしか存在しません。無くても動作は
  します（ハッシュが JavaScript 実装にフォールバックし、数倍遅くなります）が、service worker が登録されない
  ためオフライン動作は失われます。

ホストが **`.wasm` を配信できること**も確認してください。未知の拡張子をブロックする規則があると、パッチ処理は
始まる前に止まり、設定ページがその旨を表示します。
