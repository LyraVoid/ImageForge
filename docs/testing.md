...
This table is generated: the two columns only a person can write live in `scripts/materials.mjs` next to
the material they describe, and the other two — which tests read a variable, and whether anything still
does — are read out of the test tree. `pnpm docs:materials` rewrites it, `pnpm docs:materials:check`
fails when it is stale, and the test suite runs the check, so a table that drifts cannot be committed.

<!-- material-table:begin: generated by scripts/materials.mjs -->
...
| `IMAGEFORGE_BOOTANIMATION` | a real vendor boot animation archive (the one used here came out of the OTA's my_product partition) | `unit/bootanimation.test` | `.research/bootanimation/bootanimation.zip` |
